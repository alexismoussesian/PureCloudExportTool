...
namespace pcet.plugin.sql.DataModel.DataTableRow
{
    public class DataTableRowManager
    {
        ...
        public static void SaveGroupMembers(IList<DataTableRows> dataTableId, string connectionString)
        {
            try
            {
                ...
                foreach (var row in dataTableId)
                {
                    ...
                    if (dbCtx.DataTableRows.Any(x => x.dataTableId.Equals(row.dataTableId, StringComparison.OrdinalIgnoreCase) && x.dataTableRows.Equals(row.dataTableRows, StringComparison.OrdinalIgnoreCase)))
                    {
                        counter.RowsIgnored++;
                        continue;
                    }
